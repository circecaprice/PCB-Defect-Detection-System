...
import sys
print("[camera_driver] running under Python:", sys.executable)
print("[camera_driver] sys.path[0..3]:", sys.path[:4])

import zwoasi as asi   # let the real error surface
print("[camera_driver] zwoasi imported from:", asi.__file__)
'''
...
class ZWOCamera:
    ...
    def __init__(self):
        ...
        self.controls = None
        self.current_bin = 1
        self.last_exposure_us = 10_000  # cached so the live thread can size its timeout
        self.last_frame_bytes = 0       # cached frame size so the live thread can size its timeout
    ...
    def set_exposure_us(self, exposure_us):
        self.last_exposure_us = int(exposure_us)
    ...
    def set_roi(self, bin_val=1, image_type="RAW8"):
        ...
        # bytes per pixel for the chosen image type (used to estimate USB transfer time)
        bytes_per_px = {"RAW8": 1, "Y8": 1, "RAW16": 2, "RGB24": 3}.get(image_type.upper(), 1)
        self.last_frame_bytes = w * h * bytes_per_px
